...
To build the Windows portable tree, run `pnpm run init` and then `pnpm package:win:portable`. The output is `release/win-unpacked`. Perform the first-run test on a clean computer or real USB drive and verify that wallet state exists only under the package's own `data/clawx-state` directory. If an older host wallet exists under `%APPDATA%/clawx`, the portable UI asks before importing it and never silently creates a competing wallet; an already-running managed Gateway is restarted in the background after the wallet and provider configuration is persisted. Disposable Chromium session caches and logs use the host cache (`%LOCALAPPDATA%/U-Claw`) by default to avoid slow USB random writes. See [docs/portable-storage-boundary.md](docs/portable-storage-boundary.md) and validate the ShadowFork contract with `pnpm run shadowfork:contract`.
