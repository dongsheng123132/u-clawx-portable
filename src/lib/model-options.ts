...
export function buildConfiguredModelOptions(
  ...
  for (const account of entries) {
    ...
    const modelIds = (() => {
      ...
      // uclaw-cloud 是我们自己的聚合网关，一把 key 后面挂着多个模型 —— 和 custom/ollama
    // 同理，要把整份清单摆进下拉，而不是只露 account.model 那一个。
    // 顺序即展示顺序（下面按 configured 的数组序去重），首推的排前面。
    const supportsMultipleModels = account.vendorId === 'custom'
      || account.vendorId === 'ollama'
      || account.vendorId === 'uclaw-cloud';
