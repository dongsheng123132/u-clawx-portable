...
describe('openclaw bundle config', () => {
  it('pins the OpenClaw 2026.7.1-2 runtime compatibility matrix', () => {
    ...
    expect(packageJson.devDependencies?.['@openclaw/whatsapp']).toBeUndefined();
    expect(packageJson.devDependencies?.['@whiskeysockets/baileys']).toBeUndefined();
  ...
  it('includes Electron runtime-only packages needed in packaged builds', async () => {
    ...
    expect(ELECTRON_MAIN_RUNTIME_PACKAGES).toEqual(['qrcode-terminal']);
    ...
    expect(EXTRA_BUNDLED_PACKAGES).not.toContain('@whiskeysockets/baileys');
    const packageJson = JSON.parse(readFileSync(resolve(process.cwd(), 'package.json'), 'utf8')) as {
      dependencies?: Record<string, string>;
      devDependencies?: Record<string, string>;
    };
