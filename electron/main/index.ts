...
import { ensureUclawCloudAccount } from '../services/uclaw-cloud-account';
...
// 🔴 便携模式：把 Electron 的 userData 改道到 <appRoot>/data/clawx-state。
//
// **必须在 app.whenReady() 之前、任何窗口和 electron-store 创建之前**做 ——
...
function createWindow(): BrowserWindow {
  ...
  installEditContextMenu(win.webContents);
...
async function initialize(): Promise<void> {
  ...
  // 虾盘云：确保这台机器有设备钱包，并把凭证配到 uclaw-cloud provider 上。
  // **不 await** —— 它要联网，阻塞启动会让 U 盘首启更慢；而且它内部
  // 绝不抛异常，拿不到凭证也只是「这次没自动配上」，用户照样能进应用。
  void ensureUclawCloudAccount(gatewayManager).catch(() => {});
