...
import { getOpenClawDir, getOpenClawEntryPath, getOpenClawPortableEnv} from '../utils/paths';
...
export async function runOpenClawDoctorRepair(): Promise<boolean> {
  ...
  return await new Promise<boolean>((resolve) => {
    ...
    const child = utilityProcess.fork(entryScript, doctorArgs, {
      cwd: openclawDir,
      stdio: 'pipe',
      // 便携：doctor 也是 OpenClaw runtime，不注入就会去读宿主机 ~/.openclaw
      env: { ...forkEnv, ...getOpenClawPortableEnv() } as NodeJS.ProcessEnv,
    });
