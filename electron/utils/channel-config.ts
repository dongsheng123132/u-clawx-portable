...
import { getOpenClawConfigDir, getOpenClawCredentialsDir, getOpenClawExtensionsDir, getOpenClawResolvedDir, resolveOpenClawConfigPath } from './paths';
...
// schema validation errors.  U-ClawX falls back to DEFAULT_ACCOUNT_ID
...
// but whose account payload contains nested strict-schema objects that U-ClawX
...
// OpenClaw 2026.7.1 bundles only these channel extensions. All other U-ClawX
...
function sanitizeDiscordGuildChannelConfig(channelConfig: unknown): void {
    ...
    // Backward compatibility for the older U-ClawX-generated shape:
...
function ensurePluginRegistration(currentConfig: OpenClawConfig, pluginId: string): void {
    ...
    // accounts. Older U-ClawX versions mirrored credentials here, which OpenClaw
...
export async function deleteAgentChannelAccounts(agentId: string, ownedChannelAccounts?: Set<string>): Promise<void> {
    ...
    await mutateOpenClawConfig((snapshot) => {
        ...
        // Older U-ClawX releases could leave the only copy of Discord, QQBot,
