...
export async function ensureOpenClaw2026_7_1UpgradeSnapshot(
  ...
  try {
    ...
    for (const databasePath of [
      ...
      const relativeDatabase = toPortableRelativePath(relative(stateDir, databasePath));
    ...
    const files = copiedDestinations
      .map((path) => toPortableRelativePath(relative(tempDir, path)))
      .sort();
