...
import { getOpenClawDir, getOpenClawEntryPath, getOpenClawPortableEnv } from './paths';
...
export function getOpenClawCliSpawnSpec(): OpenClawCliSpawnSpec {
  ...
  if (app.isPackaged) {
    ...
    return {
      ...
      args: [entryPath],
      env: { ...process.env, ...getOpenClawPortableEnv(), ELECTRON_RUN_AS_NODE: '1' },
    };
  }
...
function ensureWindowsCliOnPath(): Promise<'updated' | 'already-present'> {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      {
        env: { ...process.env, ...getOpenClawPortableEnv() },
