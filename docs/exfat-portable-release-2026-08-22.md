...
1. **设备钱包端到端从未真机验收**：粘贴 key → 启用 → 余额正确 → 发消息能扣费，这条链
   一次都没跑通过。#4 的修复也因此**只有单测覆盖，没有真机验证**。
2. **28 个存量单测失败**，是这次 upstream 同步遗留的，本次改动之前就红着（基线
...
3. **老版本为什么没暴露 #2**，没查清。已排除「老运行时没这机制」—— 2026.6.10 同样有
