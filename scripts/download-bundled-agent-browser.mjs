...
async function setupTarget(id) {
  ...
  // Package resource cache policy: restore from .cache/agent-browser first.
  // GitHub releases are unreachable from CN networks without a proxy, and the
  // packaging pipeline consumes resources/bin between runs, so a hard
  // re-download on every package run makes packaging flaky.
  const cachedBin = path.join(OUTPUT_BASE, '..', '..', '.cache', 'agent-browser', target.asset);
  if (!process.argv.includes('--force') && !(await fs.pathExists(destBin)) && await fs.pathExists(cachedBin)) {
    await fs.ensureDir(targetDir);
    await fs.copy(cachedBin, destBin);
    echo(chalk.green(`   ✅ Restored ${id} from package cache`));
  }
  if (!process.argv.includes('--force') && await fs.pathExists(destBin)) {
    const { size: existingSize } = await fs.stat(destBin);
    if (existingSize > 1024 * 1024) {
      echo(chalk.green(`   ✅ Reusing existing ${id}: ${formatBytes(existingSize)} (pass --force to re-download)`));
      installed.push({ id, asset: target.asset, dest: destBin, size: existingSize });
      return;
    }
  }
