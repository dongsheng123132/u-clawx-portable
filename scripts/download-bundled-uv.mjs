...
async function setupTarget(id) {
  ...
  try {
    ...
    if (archive.source === 'cache') {
      echo(chalk.cyan`📦 Using cached uv archive: ${archivePath}`);
    } else if (archive.source === 'stale-cache') {
      echo(chalk.yellow`⚠️  Download failed; using stale cached uv archive: ${archivePath}`);
      echo(chalk.yellow`   ${archive.warning}`);
    } else {
      echo(chalk.green`⬇️ Cached uv archive: ${archivePath}`);
    }
