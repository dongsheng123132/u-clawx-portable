...
import 'zx/globals';
import { getOrDownloadCachedFile } from './package-resource-cache.mjs';

const ROOT_DIR = path.resolve(__dirname, '..');
// Must be >= 22.22.3: besides parsing TC39 Explicit Resource Management,
// OpenClaw requires a Node build with a WAL-reset-safe SQLite runtime.
// Node 22.20.0 is rejected at startup because its embedded SQLite is unsafe.
...
async function setupTarget(id) {
  ...
  try {
    const archive = await getOrDownloadCachedFile({
      url: downloadUrl,
      rootDir: ROOT_DIR,
      namespace: 'node',
    ...
    if (archive.source === 'cache') {
      echo(chalk.cyan`📦 Using cached Node.js archive: ${archivePath}`);
    } else if (archive.source === 'stale-cache') {
      echo(chalk.yellow`⚠️  Download failed; using stale cached Node.js archive: ${archivePath}`);
      echo(chalk.yellow`   ${archive.warning}`);
    } else {
      echo(chalk.green`⬇️ Cached Node.js archive: ${archivePath}`);
    }
